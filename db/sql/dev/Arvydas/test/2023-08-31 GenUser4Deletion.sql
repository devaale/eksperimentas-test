...
SET @userId = NEWID()
SET @email = CONCAT(@userId, '@energus.eu')

PRINT 'NEW USER, Id: ' + @userId +', Email: ' + @email
...
IF EXISTS (SELECT * FROM AspNetUsers WHERE Id = @userId) BEGIN
...
SELECT Name, RemovalRequested FROM AspNetUsers
