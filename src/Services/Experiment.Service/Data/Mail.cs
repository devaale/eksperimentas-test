...
using Experiment.Service.Config;

namespace Experiment.Service.Data{
    public class Mail
    {
        /// <summary>
        ...
        public Dictionary<string, string> MailSettings()
        {
            ...
            if (!_MailSettings.Any())
            {
                ...
                var wStep = string.Format("{0}, Default mail settings loaded", wLocation);
                _Logger.WriteLine(5, wStep);
            }
        ...
        static void LoadAndParseSettings(string path)
        {
            try
            {
                ...
                var wStep = string.Format("{0}, Mail settings loaded from config file: {1}", wLocation, DEFAULT_MAIL_CONFIG_PATH);
                _Logger.WriteLine(5, wStep);
            }
            catch (Exception ex)
            {
                var wStep = string.Format("{0}, Mail config file problem", wLocation);
                _Logger.WriteLine(5, wStep);
            }
        }
    }
